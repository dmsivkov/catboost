...
namespace {
    struct TFunctionWrapper : OMPNPar::ILocallyExecutable {
        OMPNPar::TLocallyExecutableFunction Exec;
        TFunctionWrapper(OMPNPar::TLocallyExecutableFunction exec)
            : Exec(std::move(exec))
        {
        }
    ...
    class TFunctionWrapperWithPromise: public OMPNPar::ILocallyExecutable {
    private:
        OMPNPar::TLocallyExecutableFunction Exec;
        int FirstId, LastId;
    ...
    public:
        TFunctionWrapperWithPromise(OMPNPar::TLocallyExecutableFunction exec, int firstId, int lastId)
            : Exec(std::move(exec))
            , FirstId(firstId)
            , LastId(lastId)
        {
    ...
    struct TSingleJob {
        TIntrusivePtr<OMPNPar::ILocallyExecutable> Exec;
        int Id{0};

        TSingleJob() = default;
        TSingleJob(TIntrusivePtr<OMPNPar::ILocallyExecutable> exec, int id)
            : Exec(std::move(exec))
            , Id(id)
        {
        }
    ...
    class TLocalRangeExecutor: public OMPNPar::ILocallyExecutable {
        TIntrusivePtr<OMPNPar::ILocallyExecutable> Exec;
        TAtomic Counter;
        TAtomic WorkerCount;
        int LastId;
...
//////////////////////////////////////////////////////////////////////////
class OMPNPar::TLocalExecutor::TImpl {
public:
...
OMPNPar::TLocalExecutor::TImpl::~TImpl() {
...
void* OMPNPar::TLocalExecutor::TImpl::HostWorkerThread(void* p) {
...
bool OMPNPar::TLocalExecutor::TImpl::GetJob(TSingleJob* job) {
...
void OMPNPar::TLocalExecutor::TImpl::RunNewThread() {
...
void OMPNPar::TLocalExecutor::TImpl::LaunchRange(TIntrusivePtr<TLocalRangeExecutor> rangeExec,
                                              int queueSizeLimit,
                                              TAtomic* queueSize,
                                              TLockFreeQueue<TSingleJob>* jobQueue) {
...
OMPNPar::TLocalExecutor::TLocalExecutor()
    : Impl_{MakeHolder<TImpl>()} {
}

OMPNPar::TLocalExecutor::~TLocalExecutor() = default;

void OMPNPar::TLocalExecutor::RunAdditionalThreads(int threadCount) {
    for (int i = 0; i < threadCount; i++)
        Impl_->RunNewThread();
}

void OMPNPar::TLocalExecutor::Exec(TIntrusivePtr<ILocallyExecutable> exec, int id, int flags) {
...
void OMPNPar::TLocalExecutor::Exec(TLocallyExecutableFunction exec, int id, int flags) {
#if defined(USE_OMP)
    flags = 0;
    exec(id);
#else
...
void OMPNPar::TLocalExecutor::ExecRange(TIntrusivePtr<ILocallyExecutable> exec, int firstId, int lastId, int flags) {
...
void OMPNPar::TLocalExecutor::ExecRange(TLocallyExecutableFunction exec, int firstId, int lastId, int flags) {
#if defined(USE_OMP)
    flags = 0;
    if (lastId - firstId == 1) {
        exec(firstId);
...
void OMPNPar::TLocalExecutor::ExecRangeWithThrow(TLocallyExecutableFunction exec, int firstId, int lastId, int flags) {
#if defined(USE_OMP)
    flags = 0;
    if (lastId - firstId == 1) {
        exec(firstId);
...
OMPNPar::TLocalExecutor::ExecRangeWithFutures(TLocallyExecutableFunction exec, int firstId, int lastId, int flags) {
...
void OMPNPar::TLocalExecutor::ClearLPQueue() {
    for (bool cont = true; cont;) {
        cont = false;
        TSingleJob job;
...
int OMPNPar::TLocalExecutor::GetQueueSize() const noexcept {
    return AtomicGet(Impl_->QueueSize);
}

int OMPNPar::TLocalExecutor::GetMPQueueSize() const noexcept {
    return AtomicGet(Impl_->MPQueueSize);
}

int OMPNPar::TLocalExecutor::GetLPQueueSize() const noexcept {
    return AtomicGet(Impl_->LPQueueSize);
}

int OMPNPar::TLocalExecutor::GetWorkerThreadId() const noexcept {
    return Impl_->WorkerThreadId;
}

int OMPNPar::TLocalExecutor::GetThreadCount() const noexcept {
